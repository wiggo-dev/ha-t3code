...
**Current release:** add-on **0.3.3** — pin matrix, store-ready custom repo, thin Core/Supervisor REST evidence (including Core journal via `/core/logs`), Home Assistant Skills with gather→propose→operator-applies guidance.
...
Then install **T3 Code**, Update/Rebuild after repo changes, and confirm the startup log shows `T3 Code add-on version 0.3.3` plus **Pin matrix** lines.
