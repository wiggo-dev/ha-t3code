...
3. **ACP authenticate** — At session start T3 sends ACP `authenticate` with `methodId: "cursor_login"` ([`CursorAcpSupport.ts`](https://github.com/pingdotgg/t3code/blob/main/apps/server/src/provider/acp/CursorAcpSupport.ts)). **Caveat (upstream):** as of t3 0.0.33, API-key-only sessions are treated as logged out because the probe keys off `agent about` `userEmail`, and `cursor_login` may fail with `Invalid params` when no browser is available ([t3code#7244](https://github.com/pingdotgg/t3code/issues/7244)). Workaround for this Add-on: browser `cursor-agent login` once (file store under Provider home) in addition to optional `CURSOR_API_KEY`.
4. **Status surfacing** — `cursor-agent about` JSON may include `subscriptionTier`; T3 maps it to provider auth metadata (e.g. “Cursor Pro Subscription”) ([`parseCursorAboutOutput`](https://github.com/pingdotgg/t3code/blob/main/apps/server/src/provider/Layers/CursorProvider.ts)). Empty/`null` email ⇒ unauthenticated in T3 even when the API key works for the CLI.
